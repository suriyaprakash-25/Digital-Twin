    ...
    <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <header className="mb-4 md:mb-8">
        <h1 className="text-2xl md:text-4xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
          <UserPlus className="h-6 w-6 md:h-8 md:w-8 text-teal-600" />
          Transfer Ownership
        </h1>
        {vehicle && (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl md:rounded-3xl p-4 md:p-8 shadow-sm">
        <div className="bg-amber-50 border border-amber-150 rounded-xl md:rounded-2xl p-3.5 md:p-5 mb-4 md:mb-6 flex gap-3 text-amber-800 text-xs md:text-sm font-semibold">
          <AlertTriangle className="h-5 w-5 text-amber-600 shrink-0 mt-0.5" />
          <div className="space-y-1.5 leading-relaxed">
            <h4 className="font-bold text-sm md:text-base text-amber-950">Important Notice</h4>
            <p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-1.5">
            <label className="text-xs md:text-sm font-bold text-slate-700">Buyer Email Address</label>
            <div className="relative group">
              ...
              <input
                ...
                required
                className="block w-full pl-9 md:pl-11 pr-4 py-2 md:py-3 border border-slate-200 rounded-xl text-xs md:text-sm text-slate-900 bg-slate-50 focus:bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all font-medium hover:border-slate-300 shadow-sm"
              />
            </div>
            <p className="text-[10px] text-slate-400 font-medium">
              The buyer must be registered on the platform with this exact email address.
            </p>
          </div>

          <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-[10px] md:text-xs text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full sm:w-auto px-4 py-2.5 md:px-8 md:py-3.5 bg-teal-600 hover:bg-teal-700 hover:text-white text-white font-bold rounded-xl transition-all shadow-sm hover:shadow-md disabled:opacity-50 text-xs md:text-sm"
            >
