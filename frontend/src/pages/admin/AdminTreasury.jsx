...
export default function AdminTreasury() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-slate-900">Treasury Liquidity & Settlement Intelligence</h1>
              <p className="text-xs text-slate-500">Automated payout forecasting, settlement velocity, aging buckets, and SLA breach monitoring</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-2xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-black uppercase tracking-wider text-slate-900">Settlement Aging Distribution</h2>
          </div>
          <span className="text-xs text-slate-500">Total Open: {aging?.totalOpenSettlements || 0}</span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          {aging?.bucketSummaries && Object.entries(aging.bucketSummaries).map(([bucketKey, bData]) => (
            ...
            >
              ...
              <span className="text-lg font-black text-slate-900 block">₹{bData.amountRupees.toLocaleString('en-IN')}</span>
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">{aging.slaBreaches.length} settlement(s) currently exceed configured review or processing SLA limits.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden space-y-4">
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-cyan-400" />
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-900">Settlement Aging Queue</h3>
          </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                ...
                filteredRecords.map((r) => (
                  <tr key={r.settlementId} className="hover:bg-slate-100/40 transition">
                    <td className="px-6 py-3.5 font-mono font-semibold text-cyan-400">{r.settlementId}</td>
                    <td className="px-6 py-3.5 text-slate-900 font-medium">{r.garageName}</td>
                    <td className="px-6 py-3.5 font-bold text-slate-900">₹{r.amountRupees.toLocaleString('en-IN')}</td>
                    <td className="px-6 py-3.5">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600">
                        {r.status}
                      </span>
                    </td>
                    <td className="px-6 py-3.5 text-slate-500">{r.ageDays} days ({r.ageHours}h)</td>
                    <td className="px-6 py-3.5">
